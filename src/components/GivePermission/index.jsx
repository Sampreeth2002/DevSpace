...
function GivePermission(props) {
	...
	const onSubmitGivePermission = async (event, doctorID) => {
		...
		const res = await ctx.medicalSystem.methods.givePermission(doctorID).send({ from: ctx.account })
			...
			<div style={{ textAlign: "center", width: "800px" }}>
				...
				<Grid container spacing={5} direction='row' justify='flex-start' alignItems='flex-start'>
					{Object.values(otherDoctors).map(doctor => (
						<Card style={{ width: "250px", marginRight: "18px" }} className={classes.root}>
							<CardActionArea>
								<CardMedia
									component='img'
									alt='Doctor Image'
							...
							<form className={classes.container} noValidate autoComplete='off'>
								...
								<CardActions>
									<Button
										size='small'
										color='primary'
										onClick={event => onSubmitGivePermission(event, doctor.id)}
									>
										Give Permission
									</Button>
								</CardActions>
