...
contract("MedicalSystem", ([deployer, patient, doctor]) => {
	...
	describe("testing sytem", async () => {
		...
		before(async () => {
			...
			doctorCreated = await system.createDoctor("Ritvik", "Image URL", { from: doctor })
		...
		it("doctor created", async () => {
			...
			assert.equal(event.imageUrl, "Image URL", "incorrect image")
		})

		it("record uploaded", async () => {
			const result = await system.uploadPatientRecord("www.sampreeth.com", "Sampreeth's Report", {
				from: patient,
			})
			...
			assert.equal(event.name, "Sampreeth's Report", "incorrect record name uploaded")
			assert.closeTo(
		...
		it("fetching the patient record", async () => {
			...
			assert.equal(
				result.name,
				"Sampreeth's Report",
				"record fetched has incorrect name after uploading"
			)
